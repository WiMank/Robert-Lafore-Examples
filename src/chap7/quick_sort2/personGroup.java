    ...
    public void draw(Graphics var1) {
        ...
        if (this.drawMode != 2) {
            if (this.codePart == 3) {
                ...
                this.drawOneBar(var1, this.center);
                this.drawOneBar(var1, this.right - 1);
            }

            if (this.codePart != 15 && this.codePart != 7) {
                if (this.codePart == 6) {
                    this.drawOneBar(var1, this.pivot);
                    this.drawOneBar(var1, this.leftScan);
                }
            } else {
                this.drawOneBar(var1, this.right);
                this.drawOneBar(var1, this.left);
                this.drawOneBar(var1, this.center);
            }
        } else {
            var1.setColor(Color.lightGray);
    ...
    public void sortStep() {
        if (!this.doneFlag) {
            switch(this.codePart) {
                case 0:
                    this.note = "quickSort entry: ";
                    this.size = this.right - this.left + 1;
                    if (this.size == 1) {
                        this.note = this.note + "Array of 1 (" + this.left + "-" + this.right + ") always sorted";
                    } else if (this.size == 2) {
                        this.note = this.note + "Will sort 2-element array (" + this.left + "-" + this.right + ")";
                    } else {
                        this.center = (this.left + this.right) / 2;
                        this.note = this.note + "Will sort left, center, right (" + this.left + "-" + this.center + "-" + this.right + ")";
                    }
                ...
                case 1:
                    if (this.size == 1) {
                        this.note = "No action necessary";
                        this.codePart = 7;
                        return;
                    } else if (this.size == 2) {
                        ++this.comps;
                        if (this.theArray[this.left].getHeight() > this.theArray[this.right].getHeight()) {
                            this.swap(this.left, this.right);
                        }

                        this.note = "Done 2-element sort";
                        this.codePart = 7;
                        return;
                    } else {
                        if (this.theArray[this.left].getHeight() > this.theArray[this.center].getHeight()) {
                            this.swap(this.left, this.center);
                        }
                        ...
                        if (this.theArray[this.center].getHeight() > this.theArray[this.right].getHeight()) {
                            this.swap(this.center, this.right);
                        }

                        this.comps += 3;
                        if (this.size == 3) {
                            this.note = "Done left-center-right sort";
                            this.codePart = 7;
                            return;
                        }

                        this.pivot = this.center;
                        this.note = "Will partition (" + this.left + "-" + this.right + "); pivot will be " + this.pivot;
                        this.codePart = 15;
                        return;
                    }
                case 2:
                    this.swap(this.center, this.right - 1);
                    this.pivot = this.right - 1;
                    this.leftScan = this.left;
                    this.rightScan = this.right - 1;
                    this.note = "Will scan (" + (this.leftScan + 1) + "-" + (this.rightScan - 1) + ")";
                    this.codePart = 3;
                    return;
                case 3:
                    ++this.comps;
                ...
                case 7:
                    ...
                    this.rightScan = this.right - 1;
                    this.pivot = this.right;
                    this.note = "Will call quickSort for right partition (" + this.left + "-" + this.right + ")";
                    this.codePart = 0;
                    return;
                case 8:
                    this.codePart = 8;
                    ...
                    return;
                case 15:
                    this.note = "Will swap pivot and right-1";
                    this.codePart = 2;
                    return;
                default:
            }
